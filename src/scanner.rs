...
use crate::fs_utils::{
    get_fs_type, is_broken_symlink, resolve_target_path, scan_directory_recursive,
};
...
impl Scanner {
    ...
    pub fn scan_targets(&self, config: &DotyConfig, lockfile: &Lockfile) -> Result<Vec<DriftItem>> {
        let mut drift_items = Vec::new();

        // Scan each package for drift
        for package in &config.packages {
            let package_drift = self.scan_package(package, config, lockfile)?;
            ...
            LinkStrategy::LinkFilesRecursive => {
                ...
                if source_path.is_dir() {
                    ...
                    for target_file in target_files {
                        let relative_path =
                            target_file.strip_prefix(&target_path).with_context(|| {
                                format!("Failed to get relative path for {}", target_file)
                            })?;
...
mod tests {
    ...
    fn setup_test_env() -> Result<(TempDir, Utf8PathBuf, DotyConfig, Lockfile)> {
        ...
        // Create lockfile
        let lockfile = Lockfile::new("test-host".to_string(), temp_path.clone());

        Ok((temp_dir, temp_path, config, lockfile))
    }

    #[test]
    fn test_scan_link_files_recursive_untracked_files() -> Result<()> {
        let (_temp_dir, temp_path, mut config, lockfile) = setup_test_env()?;
        ...
        let drift_items = scanner.scan_targets(&config, &lockfile)?;
    ...
    fn test_scan_link_folder_no_untracked_detection() -> Result<()> {
        ...
        let untracked_count = drift_items
            .iter()
    ...
    fn test_scan_broken_symlinks() -> Result<()> {
        let (_temp_dir, temp_path, mut config, mut lockfile) = setup_test_env()?;
        ...
        // Add to lockfile
        lockfile.add_link(target_file.clone(), Utf8PathBuf::from("test-app.txt"));
    ...
    fn test_scan_mixed_scenarios() -> Result<()> {
        ...
        lockfile.add_link(broken_target.clone(), Utf8PathBuf::from("missing"));
        ...
        let untracked_items: Vec<_> = drift_items
            .iter()
            .filter(|item| item.drift_type == DriftType::Untracked)
            .collect();
        let broken_items: Vec<_> = drift_items
            .iter()
    ...
    fn test_scan_broken_symlinks_relative_path_different_cwd() -> Result<()> {
        let (_temp_dir, temp_path, config, mut lockfile) = setup_test_env()?;
        ...
        // Add to lockfile using RELATIVE path
        // target/test-app.txt relative to temp_path
        let relative_target = Utf8PathBuf::from("target/test-app.txt");
        lockfile.add_link(relative_target.clone(), Utf8PathBuf::from("test-app.txt"));

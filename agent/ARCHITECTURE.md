...
To safely manage symlinks (especially deletions), Doty tracks the state of deployed links.
...
*   **Description**: Applies the configuration, creating symlinks based on the chosen strategy.
*   **Options**:
    *   `--dry-run`: Simulates changes (creations/deletions) without modifying the filesystem.
*   **Logic**:
...
*   **Logic**: Uses `.doty/state/<hostname>.kdl` to identify and remove only Doty-managed links.

### 4.3 `doty adopt`
*   **Description**: Interactive wizard to import existing local configs into the Doty repo.
*   **Workflow**:
...
*   **Interactive Mode**:
    *   If untracked files are found: Ask to **Adopt** them (trigger `doty adopt` logic).
...
*   [ ] **Scanner**: Implement logic to scan targets and compare with Source/State.

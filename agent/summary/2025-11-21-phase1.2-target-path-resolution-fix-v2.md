...
- **`pathResolution: config`**: Relative paths should be resolved relative to the config file's directory (repo_root)
...
fn resolve_target_path(&self, target: &Utf8Path) -> Result<Utf8PathBuf> {
    ...
    match self.path_resolution {
        PathResolution::Config => {
            // Resolve relative to repo_root (config file's directory)
            repo_root + target
        }
        PathResolution::Cwd => {

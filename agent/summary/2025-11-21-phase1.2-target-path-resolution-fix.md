...
- Updated `Linker::new()` to only take `config_dir_or_cwd` parameter
...
- Updated `Linker::new()` calls to only pass `config_dir_or_cwd`
...
- Updated `setup_test_fs()` to only return `config_dir_or_cwd`
